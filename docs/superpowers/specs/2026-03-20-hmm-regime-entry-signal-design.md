...
Alias resolution happens during the formula rewriting pass (see below), not in the parser. The pre-evaluation pass replaces `hmm_regime(SPY, 3, 5) == bullish` with `__hmm_regime_SPY_3_5 == 2` before the formula reaches the tokenizer.
...
3. Rewrite the formula string, replacing the entire `hmm_regime(...) == alias` expression with `__hmm_regime_SPY_3_5 == 2`
...
```
hmm_regime(3, 5)         →  __hmm_regime_AAPL_3_5    (primary symbol = AAPL)
hmm_regime(SPY, 3, 5)    →  __hmm_regime_SPY_3_5
```
...
→ __hmm_regime_SPY_3_5 == 2 and rsi(close, 14) < 30
```
...
The parser's `parse_primary()` treats unknown identifiers as cross-symbol references — uppercasing them and appending `_close` (e.g., `SPY` becomes `col("SPY_close")`). This would mishandle `__hmm_regime_SPY_3_5`, producing `col("__HMM_REGIME_SPY_3_5_close")` instead of `col("__hmm_regime_SPY_3_5")`.
...
**Known behavior:** The forward filter can produce rapid regime switching on boundary days where the posterior is near 50/50 between states. This is inherent to online filtering without smoothing. Users who want more stable regime labels should use fewer states (n_regimes=2) or combine with other signals.
...
3. Scan formula for hmm_regime() calls                (NEW)
   → Extract: [("SPY", 3, 5)]
   → Rewrite formula: "__hmm_regime_SPY_3_5 == 2 and rsi(close, 14) < 30"
4. For each unique (symbol, n_regimes, fit_years):    (NEW)
   a. Load symbol OHLCV from cache
   b. Split at backtest start_date
   c. Fit HMM on pre-backtest returns (min 50 bars)
   d. Forward-filter classify backtest bars
   e. Inject __hmm_regime_SPY_3_5 column into primary DF
...
7. build_signal() → parse rewritten formula → Expr    (existing)
   __hmm_regime_SPY_3_5 resolves to col(...)
   2 resolves to lit(2)
...
`__hmm_regime_{SYMBOL}_{N_REGIMES}_{FIT_YEARS}` (e.g., `__hmm_regime_SPY_3_5`)
...
- If the same `(symbol, n_regimes, fit_years)` tuple appears in both entry and exit signals, the column is computed once and reused

## Code Changes

| File | Change |
|------|--------|
| `engine/hmm.rs` | Add `forward_filter()` function (log-space, returns `Vec<usize>`) |
...
- Rewriting replaces entire `hmm_regime(...) == bullish` with `__hmm_regime_SPY_3_5 == 2`
...
- Injected column has correct name (`__hmm_regime_SPY_3_5`)
