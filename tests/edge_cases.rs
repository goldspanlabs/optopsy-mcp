...
fn metrics_with_no_trades_returns_defaults() {
    let result =
        optopsy_mcp::engine::metrics::calculate_metrics(&[], &[], 10_000.0, 252.0).unwrap();
...
fn metrics_all_fields_are_finite() {
    ...
    let m = optopsy_mcp::engine::metrics::calculate_metrics(&equity, &[trade], 10_000.0, 252.0)
        .unwrap();
...
fn signal_exit_fires_mid_position() {
    ...
    params.exit_signal = Some(optopsy_mcp::signals::registry::SignalSpec::Formula {
