...
fn execute_update(
    ...
    match storage::update_signal(old_name, new_name, display_name, formula, None) {
        Ok(()) => base_response(
            ...
            None,
            vec![
                format!("[INFO] Signal is now available as '{new_name}'"),
                "[NEXT] Use build_signal action='list' to see all signals".to_string(),
            ],
        ),
        Err(e) => base_response(
            format!("Failed to rename signal: {e}"),
            false,
            None,
            vec![],
            None,
            vec!["[RETRY] Check signal names with action='list'".to_string()],
        ),
    }
}
