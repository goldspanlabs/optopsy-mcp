...
#[test]
fn test_transpile_multi_symbols() {
    let dsl = r#"
strategy "Multi"
  symbols SPY, QQQ
  interval daily
...
"#;
    ...
    // Two extern_symbol bindings
    assert!(rhai.contains(r#"extern_symbol("spy", "SPY", "ticker")"#));
    assert!(rhai.contains(r#"extern_symbol("qqq", "QQQ", "ticker")"#));

    // First symbol aliased as `symbol`
    assert!(rhai.contains("let symbol = spy;"));

    // buy_stock uses symbol
    assert!(rhai.contains("buy_stock(symbol, 10)"));
}

#[test]
fn test_transpile_multi_symbols_case_normalized() {
    let dsl = r#"
strategy "Multi"
  symbols spy, qqq
  interval daily
...
"#;
    ...
    // Lowercase input should be normalized to uppercase defaults
    assert!(rhai.contains(r#"extern_symbol("spy", "SPY", "ticker")"#));
    assert!(rhai.contains(r#"extern_symbol("qqq", "QQQ", "ticker")"#));
}

#[test]
fn test_transpile_multi_symbols_duplicate_error() {
    let dsl = r#"
strategy "Multi"
  symbols SPY, SPY
  interval daily
...
"#;
    ...
    let err = transpile(dsl).unwrap_err();
    assert!(
        err.to_string().contains("duplicate symbol"),
        "Should error on duplicate: {err}"
    );
}

#[test]
fn test_transpile_multi_symbols_case_insensitive_duplicate_error() {
    let dsl = r#"
strategy "Multi"
  symbols SPY, spy
  interval daily
...
"#;
    ...
    assert!(
        err.to_string().contains("duplicate symbol"),
        "Should error on case-insensitive duplicate: {err}"
    );
}

#[test]
fn test_transpile_multi_symbols_single_entry_error() {
    let dsl = r#"
strategy "Multi"
  symbols SPY
  interval daily
...
"#;
    ...
    assert!(
        ...
        "Should error on single symbol in symbols: {err}"
    );
}
